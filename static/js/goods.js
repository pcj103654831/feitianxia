    ...
    $.ftxia.goods = {
        ...
        goods_add: function(){
					...
					if (link != '') {
						 $.getJSON(FTXIAER.root + '/?m=goods&a=ajaxGetItem', {url:link}, function(result){
							if(result.status == 1){
								...
								$('#img_url').attr('src',result.data.pic_url);
			...
			$("#smt").click(function() {
				...
				if (good_inventory == '') {
					$('#good_inventory').html('销量不能为空').show();
					$(document).scrollTop($('#good_inventory').offset().top);
					return false;
				} else {
					if (number.test(good_inventory)) {
						if (good_inventory < 1) {
							$('#good_inventory').html('销量数量应该大于200').show();
							$(document).scrollTop($('#good_inventory').offset().top);
							return false;
						}
					} else {
						$('#good_inventory').html('请输入数字').show();
						$(document).scrollTop($('#good_inventory').offset().top);
						return false;
					}
				}
				...
				if (coupon_price == '' || coupon_price == 0) {
					...
					$(document).scrollTop($('#coupon_price').offset().top);
					return false;
				} else {
					if (!reg.test(coupon_price)) {
				...
				if (parseInt(coupon_price) > parseInt(price)) {
			...
			$("#intro").focus(function() {
				if ($("#intro").val() == "请务必以客观的角度描述您的宝贝，否则会影响到您宝贝的审核，字数请控制在100以内。") {
					$("#intro").val("");
					$("#intro").css("color", "#5ABBEF");
				}
			});
			$("#intro").blur(function() {
				if ($("#intro").val() == "") {
					$("#intro").val("请务必以客观的角度描述您的宝贝，否则会影响到您宝贝的审核，字数请控制在100以内。");
					$("#intro").css("color", "#5ABBEF");
				}
			});
        }
